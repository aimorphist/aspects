...
mock.module("../../src/lib/config", () => ({
  ...
  resolveRegistryUrl: (env: string | undefined, config: string | undefined) => {
    ...
    return "https://aspects.sh/api/v1";
  },
}));
...
describe("api-client", () => {
  ...
  describe("getRegistry", () => {
    test("fetches registry index", async () => {
      ...
      expect(ofetchCalls[0]!.url).toBe(
        "https://test.example.com/api/v1/registry",
      );
    });
  ...
  describe("getAspect", () => {
    test("fetches aspect by name", async () => {
      ...
      expect(ofetchCalls[0]!.url).toBe(
        "https://test.example.com/api/v1/aspects/alaric",
      );
    });
  ...
  describe("getAspectVersion", () => {
    test("fetches specific version", async () => {
      ...
      expect(result.name).toBe("alaric");
      expect(ofetchCalls[0]!.url).toBe(
        "https://test.example.com/api/v1/aspects/alaric/1.0.0",
      );
    });
    ...
    test('handles "latest" as version', async () => {
      ...
      expect(ofetchCalls[0]!.url).toBe(
        "https://test.example.com/api/v1/aspects/alaric/latest",
      );
    });
  });
  ...
  describe("error handling", () => {
    test("wraps 404 as ApiClientError", async () => {
      ...
      try {
        ...
        expect((err as InstanceType<typeof ApiClientError>).statusCode).toBe(
          404,
        );
        expect((err as InstanceType<typeof ApiClientError>).errorCode).toBe(
          "not_found",
        );
      }
    });
  ...
  describe("caching", () => {
    ...
    test("getCategories caches results", async () => {
      ofetchImpl = async () => ({
        categories: [{ id: "assistant", name: "Assistant" }],
      });
      await getCategories();
      await getCategories();
      expect(ofetchCalls).toHaveLength(1);
